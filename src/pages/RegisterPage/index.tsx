...
import { RegisterLayout, ContentsLayout, UploaderLayout } from './style';
...
const RegisterPage: FC = () => {
  ...
  const linkType = params.type === 'wish' ? 'wishlist' : 'closet';
  ...
  const handleInputChange = (
    index: number,
    field: 'name' | 'seasonTypes' | 'clothesType',
    value: any,
  ) => {
  ...
  const handleButtonClick = async () => {
    if (contents.every(content => content.name)) {
      const images = contents.map((item, index) =>
        base64toFile(item.imageBase64, `clothimage_${index}`),
      );
        ...
        {contents.map((content, index) => (
          <Content
            ...
            onSeasonChange={selectedSeason =>
              handleInputChange(index, 'seasonTypes', selectedSeason)
            }
            onCategoryChange={selectedCategory =>
              handleInputChange(index, 'clothesType', selectedCategory)
            }
          />
        ))}
      </ContentsLayout>
